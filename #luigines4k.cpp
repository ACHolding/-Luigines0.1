...
struct Cartridge {
  ...
  bool loaded = false;
  uint16_t mapper = 0; // emulation dispatch for PRG/CHR handlers
  uint16_t inesMapper = 0; // mapper number from ROM header (for UI)
  bool uxRomHeuristicFallback = false; // unknown mapper -> UxROM try-on
  ...
  static bool mapperHasNativeSupport(uint16_t m) {
    switch (m) {
    case 0:
    case 1:
    case 2:
  ...
  bool loadFromFile(const std::string &path) {
    ...
    inesMapper = mapper;
    uxRomHeuristicFallback = false;

    // MMC1-compatible boards run on our MMC1 core.
    if (inesMapper == 180) {
      mapper = 2; // UNROM-style PRG (Crazy Climber family)
    } else if (inesMapper == 155 || inesMapper == 206) {
      mapper = 1;
    } else if (!mapperHasNativeSupport(inesMapper)) {
      // FCEUX-style permissive loading: keep running most commercial/homebrew
      // boards by falling back to simple PRG banking when possible.
      if (prgBanks >= 2) {
        mapper = 2;
        uxRomHeuristicFallback = true;
      } else {
        mapper = 0;
        uxRomHeuristicFallback = (inesMapper != 0);
      }
    }
  ...
  void cpuWrite(uint16_t addr, uint8_t value) {
    ...
    case 1:
      ...
      break;
    case 2:
      mapper2PrgBank = static_cast<uint8_t>(
          (static_cast<unsigned>(value) & 0xFFU) %
          static_cast<unsigned>(std::max(1, prgBankCount16k)));
      break;
    case 3:
...
public:
  ...
  void clock() {
    ...
    // Latch VRAM address (v) at dot 1 of each visible scanline for bit-accurate
    // background compositing (matches hardware after h-scroll reload at dot 257).
    if (rendering && scanline < 240 && cycle == 1) {
      lineStartV[static_cast<size_t>(scanline)] = vramAddr;
    }
...
private:
  ...
  std::array<uint16_t, 240> lineStartV{};
  int scanline = 0;
  int cycle = 0;
  uint8_t ppuctrl = 0;
  uint8_t ppumask = 0;
  ...
  static void incrementCoarseXOn(uint16_t &v) {
    if ((v & 0x001F) == 31) {
      v &= static_cast<uint16_t>(~0x001F);
      v = static_cast<uint16_t>(v ^ 0x0400);
    } else {
      v = static_cast<uint16_t>(v + 1);
    }
  }

  void incrementCoarseX() { incrementCoarseXOn(vramAddr); }
  ...
  void renderFrame() {
    ...
    const bool bgOn = (ppumask & 0x08) != 0;
    const bool clipBgLeft = (ppumask & 0x02) != 0;

    for (int y = 0; y < kNesHeight; ++y) {
      uint16_t vv = lineStartV[static_cast<size_t>(y)];
      int pos = fineX;
      ...
      for (int x = 0; x < kNesWidth; ++x) {
        ...
        if (bgOn) {
          const int bitInTile = 7 - (pos & 7);
          const int coarseX = vv & 0x1F;
          const int coarseY = (vv >> 5) & 0x1F;
          const uint16_t ntBase = static_cast<uint16_t>(
              0x2000 + ((vv >> 10) & 0x03) * 0x0400);
          const int fineY = (vv >> 12) & 7;

          const uint16_t ntAddr =
              static_cast<uint16_t>(ntBase + coarseY * 32 + coarseX);
          const uint8_t tileIndex = ppuRead(ntAddr);

          const uint16_t attrAddr = static_cast<uint16_t>(
              ntBase + 0x03C0 + (coarseY / 4) * 8 + (coarseX / 4));
          const uint8_t attr = ppuRead(attrAddr);
          const int shift = ((coarseY & 0x02) << 1) | (coarseX & 0x02);
          const uint8_t paletteSel =
              static_cast<uint8_t>((attr >> shift) & 0x03);

          const uint16_t pattAddr = static_cast<uint16_t>(
              bgPatternBase + tileIndex * 16 + fineY);
          const uint8_t lo = ppuRead(pattAddr);
          const uint8_t hi = ppuRead(static_cast<uint16_t>(pattAddr + 8));
          const uint8_t b0 =
              static_cast<uint8_t>((lo >> bitInTile) & 1);
          const uint8_t b1 =
              static_cast<uint8_t>((hi >> bitInTile) & 1);
...
public:
  ...
  int mapperId() const { return static_cast<int>(cart.inesMapper); }
  bool mapperUxRomFallback() const { return cart.uxRomHeuristicFallback; }
...
int main(int argc, char **argv) {
  ...
  if (argc > 1) {
    if (nes.loadRom(argv[1])) {
      running = true;
      status = "Loaded m" + std::to_string(nes.mapperId()) +
               (nes.mapperUxRomFallback() ? " (~UxROM)" : "") + ": " +
               nes.path();
    } else {
    ...
    while (SDL_PollEvent(&e)) {
      ...
      if (e.type == SDL_KEYDOWN && e.key.keysym.sym == SDLK_o &&
        ...
        if (!path.empty() && nes.loadRom(path)) {
          running = true;
          status = "Loaded m" + std::to_string(nes.mapperId()) +
                   (nes.mapperUxRomFallback() ? " (~UxROM)" : "") + ": " + path;
        } else {
        ...
        if (inside(fsHit, mx, my)) {
          ...
          if (inside(fileOpen, mx, my)) {
            ...
            if (!path.empty() && nes.loadRom(path)) {
              running = true;
              status =
                  "Loaded m" + std::to_string(nes.mapperId()) +
                  (nes.mapperUxRomFallback() ? " (~UxROM)" : "") + ": " + path;
            } else {
              status = "Open ROM canceled or failed";
            }
